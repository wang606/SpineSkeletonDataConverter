...
void readShortArray(DataInput* input, std::vector<unsigned short>& array, int n) {
    array.resize(n); 
    for (int i = 0; i < n; i++) {
...
json readAttachment(DataInput* input, const json& skin, int slotIndex, const std::string& attachmentName, const json& root, bool nonessential) {
    ...
    std::string name = (flags & 8) != 0 ? readStringRef(input, root) : attachmentName; 
    ...
    switch (type) {
        ...
        case AttachmentType_Mesh: {
            ...
            readFloatArray(input, verticesLength, uvs);
            attachment["uvs"] = uvs;
...
json readSkin(DataInput* input, bool defaultSkin, const json& root, bool nonessential) {
    ...
    if (defaultSkin) {
        ...
        for (int i = 0, n = readVarint(input, true); i < n; i++) {
            ...
            skin["physics"].push_back(root["physics"][physicsIndex]["name"]);
        }
        slotCount = readVarint(input, true);
    }
...
void readTimeline(DataInput* input, json& timelines, std::string type, int frameCount, int valueCount, std::string name1 = "value", std::string name2 = "", std::string name3 = "") {
    float time = readFloat(input);
    float value1 = readFloat(input); 
    float value2 = valueCount > 1 ? readFloat(input): 0;
    float value3 = valueCount > 2 ? readFloat(input): 0;
    timelines[type] = json::array(); 
    for (int frame = 0, bezier = 0, frameLast = frameCount - 1;; frame++) {
        timelines[type].push_back(json::object());
        timelines[type].back()["time"] = time;
        timelines[type].back()[name1] = value1;
        if (valueCount > 1) timelines[type].back()[name2] = value2;
        if (valueCount > 2) timelines[type].back()[name3] = value3;
        ...
        switch (readSByte(input)) {
            case CURVE_STEPPED:
                timelines[type].back()["curve"] = "stepped"; 
                break;
            case CURVE_BEZIER:
                timelines[type].back()["curve"] = json::array();
                readBezier(input, timelines[type].back()); 
                if (valueCount > 1) readBezier(input, timelines[type].back());
                if (valueCount > 2) readBezier(input, timelines[type].back());
                break;
        }
    }
}
...
json readAnimation(const std::string& name, DataInput* input, json& root) {
    ...
    for (int i = 0, n = readVarint(input, true); i < n; i++) {
        ...
        for (int ii = 0, nn = readVarint(input, true); ii < nn; ii++) {
            ...
            if (timelineType == BONE_INHERIT) {
                animation["bones"][boneName]["inherit"] = json::array();
                for (int frame = 0; frame < frameCount; frame++) {
                    float time = readFloat(input);
                    std::string inherit = Inherit[readByte(input)];
                    animation["bones"][boneName]["inherit"].push_back(json::object());
                    animation["bones"][boneName]["inherit"].back()["time"] = time;
                    animation["bones"][boneName]["inherit"].back()["inherit"] = inherit;
                }
                continue; 
            }
            int bezierCount = readVarint(input, true); 
    ...
    for (int i = 0, n = readVarint(input, true); i < n; i++) {
        ...
        for (int frame = 0, bezier = 0;; frame++) {
            ...
            animation["ik"][ikName].back()["bendPositive"] = (flags & 8) != 0; 
            animation["ik"][ikName].back()["compress"] = (flags & 16) != 0;
            animation["ik"][ikName].back()["stretch"] = (flags & 32) != 0;
            if (frame == frameLast) break; 
    ...
    // Physics timelines. 
    animation["physics"] = json::object(); 
    for (int i = 0, n = readVarint(input, true); i < n; i++) {
        int index = readVarint(input, true) - 1; 
        std::string name = index >= 0 ? root["physics"][index]["name"] : "";
        animation["physics"][name] = json::object(); 
        for (int ii = 0, nn = readVarint(input, true); ii < nn; ii++) {
            int type = readByte(input); 
            int frameCount = readVarint(input, true);
            if (type == PHYSICS_RESET) {
                animation["physics"][name]["reset"] = json::array();
                for (int frame = 0; frame < frameCount; frame++) {
                    float time = readFloat(input);
                    animation["physics"][name]["reset"].push_back(json::object());
                    animation["physics"][name]["reset"].back()["time"] = time;
                }
                continue;
            }
            int bezierCount = readVarint(input, true); 
            switch (type) {
                case PHYSICS_INERTIA:
                    readTimeline(input, animation["physics"][name], "inertia", frameCount, 1);
                    break; 
                case PHYSICS_STRENGTH:
                    readTimeline(input, animation["physics"][name], "strength", frameCount, 1);
                    break; 
                case PHYSICS_DAMPING:
                    readTimeline(input, animation["physics"][name], "damping", frameCount, 1);
                    break;
                case PHYSICS_MASS:
                    readTimeline(input, animation["physics"][name], "mass", frameCount, 1);
                    break;
                case PHYSICS_WIND:
                    readTimeline(input, animation["physics"][name], "wind", frameCount, 1);
                    break;
                case PHYSICS_GRAVITY:
                    readTimeline(input, animation["physics"][name], "gravity", frameCount, 1);
                    break;
                case PHYSICS_MIX:
                    readTimeline(input, animation["physics"][name], "mix", frameCount, 1);
                    break;
            }
        }
    }
    if (animation["physics"].empty()) animation.erase("physics");
    ...
    if (eventCount > 0) {
        for (int i = 0; i < eventCount; i++) {
            ...
            int intValue = readVarint(input, false);
            float floatValue = readFloat(input);
            std::string stringValue = readString(input);
            animation["events"].back()["time"] = time;
            animation["events"].back()["name"] = root["eventNames"][eventIndex];
...
bool skel2json42(const std::string& skelPath, const std::string& jsonPath) {
    ...
    if (!version.starts_with("4.2")) {
        std::cout << "Skeleton version " << version << " does not match runtime version 4.2" << std::endl;
        return false; 
    }
    ...
    for (int i = 0; i < slotCount; i++) {
        ...
        if (boneIndex < 0 || boneIndex >= numBones) {
            std::cerr << "Invalid bone index for slot: " << boneIndex << std::endl;
            return false;
        }
    ...
    for (int i = 0; i < ikConstraintsCount; i++) {
        ...
        for (int j = 0; j < boneCount; j++) {
            int boneIndex = readVarint(&input, true);
            if (boneIndex < 0 || boneIndex >= numBones) {
                std::cerr << "Invalid bone index for IK constraint: " << boneIndex << std::endl;
                return false;
            }
    ...
    for (int i = 0; i < transformConstraintsCount; i++) {
        ...
        for (int j = 0; j < boneCount; j++) {
            int boneIndex = readVarint(&input, true);
            if (boneIndex < 0 || boneIndex >= numBones) {
                std::cerr << "Invalid bone index for Transform constraint: " << boneIndex << std::endl;
                return false;
            }
        ...
        int flags = readByte(&input);
        transformConstraint["skin"] = (flags & 1) != 0;
        transformConstraint["local"] = int((flags & 2) != 0);
    ...
    for (int i = 0; i < pathConstraintsCount; ++i) {
        ...
        for (int j = 0; j < boneCount; ++j) {
            int boneIndex = readVarint(&input, true);
            if (boneIndex < 0 || boneIndex >= numBones) {
                std::cerr << "Invalid bone index for Path constraint: " << boneIndex << std::endl;
                return false;
            }
    ...
    /* Physics constraints */
    json physicsConstraints = json::array();
    int physicsConstraintsCount = readVarint(&input, true);
    for (int i = 0; i < physicsConstraintsCount; ++i) {
        json physicsConstraint;
        physicsConstraint["name"] = readString(&input);
        physicsConstraint["order"] = readVarint(&input, true);
        int boneIndex = readVarint(&input, true);
        if (boneIndex < 0 || boneIndex >= numBones) {
            std::cerr << "Invalid bone index for Physics constraint: " << boneIndex << std::endl;
            return false;
        }
        ...
        flags = readByte(&input);
        if ((flags & 1) != 0) physicsConstraint["inertiaGlobal"] = true;
        if ((flags & 2) != 0) physicsConstraint["strengthGlobal"] = true;
        if ((flags & 4) != 0) physicsConstraint["dampingGlobal"] = true;
        if ((flags & 8) != 0) physicsConstraint["massGlobal"] = true;
    ...
    if (!physicsConstraints.empty())
        root["physics"] = physicsConstraints;
    ...
    for (auto& skin : root["skins"])
        for (auto& [slotName, slot] : skin["attachments"].items())
            for (auto& [attachmentName, attachment] : slot.items())
                if (attachment["type"] == "linkedmesh") {
                    int skinIndex = attachment["skinIndex"];
                    json skin = root["skins"][skinIndex];
                    attachment["skin"] = skin["name"];
                    attachment.erase("skinIndex");
                }

        ...
        for (int ii = 0, nn = readVarint(input, true); ii < nn; ii++) {
            ...
            for (int iii = 0, nnn = readVarint(input, true); iii < nnn; iii++) {
                ...
                for (int frame = 0, bezier = 0;; frame++) {
                    ...
                    switch (readSByte(input)) {
                        case CURVE_STEPPED:
                            animation["deform"][skinName][slotName][attachmentName].back()["curve"] = "stepped";
                            break;
                        case CURVE_BEZIER:
                            animation["deform"][skinName][slotName][attachmentName].back()["curve"] = json::array();
                            readBezier(input, animation["deform"][skinName][slotName][attachmentName].back());
                            break;
                    }
                }
            }
...
bool skel2json40(const std::string& skelPath, const std::string& jsonPath) {
    ...
    if (!version.starts_with("4.0")) {
        std::cout << "Skeleton version " << version << " does not match runtime version 4.0" << std::endl;
        return false; 
    }

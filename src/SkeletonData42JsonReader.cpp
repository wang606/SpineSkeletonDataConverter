...
void readTimeline(const Json& j, std::vector<TimelineFrame>& timeline, int valueNum, const std::string& key1, const std::string& key2, float defaultValue) {
...
SkeletonData readJsonData(const Json& j) {
    ...
    skeletonData.hash = j.contains("hash") ? base64ToUint64(j["hash"]) : 0;
    if (j.contains("spine")) skeletonData.version = j["spine"];
    skeletonData.x = j.value("x", 0.0f);
    skeletonData.y = j.value("y", 0.0f);
    skeletonData.width = j.value("width", 0.0f);
    skeletonData.height = j.value("height", 0.0f);
    skeletonData.referenceScale = j.value("referenceScale", 100.0f);
    skeletonData.fps = j.value("fps", 30.0f);
    if (j.contains("imagesPath")) skeletonData.imagesPath = j["imagesPath"];
    if (j.contains("audioPath")) skeletonData.audioPath = j["audioPath"];
    ...
    for (const auto& physicsJson : j["physics"]) {
        ...
        physicsData.rotate = physicsJson.value("rotation", 0.0f);
    ...
    for (const auto& [animationName, animationJson] : j["animations"].items()) {
        ...
        skeletonData.animations[animationName] = animationData;
    }

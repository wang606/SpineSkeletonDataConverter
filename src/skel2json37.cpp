...
json readAttachment(DataInput* input, const json& skin, int slotIndex, const std::string& attachmentName, const json& root, bool nonessential) {
    ...
    switch (type) {
        case AttachmentType_Region: {
            std::string path = readString(input);
            if (!path.empty())
        ...
        case AttachmentType_LinkedMesh: {
            ...
            if (!path.empty())
                attachment["path"] = path;
            attachment["color"] = readColor(input, true);
            attachment["skin"] = readString(input);
            attachment["parent"] = readString(input);
...
json readAnimation(const std::string& name, DataInput* input, json& root) {
            ...
            switch (timelineType) {
                case SLOT_ATTACHMENT: {
                    ...
                    for (int frame = 0; frame < frameCount; frame++) {
                        ...
                        std::string attachmentName = readString(input);
    ...
    for (int i = 0, n = readVarint(input, true); i < n; i++) {
        ...
        std::string skinName = root["skinNames"][skinIndex];
...
bool skel2json37(const std::string& skelPath, const std::string& jsonPath) {
    ...
    if (!version.starts_with("3.7")) {
        std::cerr << "Skeleton version " << version << " does not match runtime version 3.7" << std::endl;
        return false; 
    }
    ...
    for (int i = 0; i < slotCount; i++) {
        ...
        slot["attachment"] = readString(&input);
    ...
    /* Skins */
    json skinNames = json::array(); 
    json skins = json::object(); 
    json defaultSkin = readSkin(&input, root, nonessential);
    if (!defaultSkin.empty()) {
        skinNames.push_back("default");
        skins["default"] = defaultSkin;
    }
    for (size_t i = 0, n = (size_t)readVarint(&input, true); i < n; i++) {
        std::string skinName = readString(&input);
        skinNames.push_back(skinName);
        json skin = readSkin(&input, root, nonessential);
        skins[skinName] = skin;
    }
    if (!skins.empty())
        root["skins"] = skins;
    root["skinNames"] = skinNames;
